...
import { ChevronLeft, Phone, MapPin, Award, ArrowRight, Loader2, User, BookOpen } from 'lucide-react';
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-card rounded-3xl border border-border shadow-soft p-8 text-center">
            ...
            <p className="text-sm font-bold text-muted-foreground mb-6 uppercase tracking-widest">Roll NO: {student.rollNumber || 'N/A'}</p>
            ...
            <div className="grid grid-cols-1 gap-4 text-left">
              <div className="p-4 bg-secondary rounded-2xl border border-border/50">
                ...
                <p className="text-lg font-black text-foreground">{className}{sectionName}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-card rounded-3xl border border-border shadow-soft p-8 space-y-4">
            ...
            <div className="flex items-center gap-4 group">
              ...
              <div className="overflow-hidden">
                <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-0.5">Gender / DOB</p>
                <p className="text-sm font-bold truncate">{student.gender} / {student.dateOfBirth}</p>
              </div>
            </div>
            <div className="flex items-center gap-4 group">
              <div className="h-10 w-10 rounded-xl bg-secondary flex items-center justify-center group-hover:bg-primary/10 transition-colors">
              ...
              <div>
                <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-0.5">Guardian Contact</p>
            ...
            <div className="flex items-center gap-4 group">
              ...
              <div>
                ...
                <p className="text-sm font-bold">{student.addressDetails?.street}, {student.addressDetails?.city}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="bg-card rounded-3xl border border-border shadow-soft p-8 flex flex-col border-b-4 border-b-amber-400">
              ...
              <h3 className="text-xl font-black tracking-tight mb-2">Academic Summary</h3>
              <p className="text-sm text-muted-foreground font-medium mb-6">Enrolled in Class {className} - Section {sectionName}.</p>

              <div className="space-y-3 mt-auto">
                <div className="flex justify-between items-center p-3 bg-secondary rounded-xl">
                  <span className="text-xs font-bold text-muted-foreground">Blood Group</span>
                  <span className="text-sm font-black">{student.bloodGroup}</span>
                </div>
                <div className="flex justify-between items-center p-3 bg-secondary rounded-xl">
                  <span className="text-xs font-bold text-muted-foreground">Father's Name</span>
                  <span className="text-sm font-black">{student.parentDetails?.fatherName}</span>
                </div>
              </div>
            </div>
          </div>
