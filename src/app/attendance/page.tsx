...
export default function AttendancePage() {
  ...
  const presentToday = students?.filter(s => (s.attendanceRate || 0) > 90).length || 0;
  ...
  const columns = [
    {
      ...
      cell: (student: Student) => (
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="font-bold text-foreground tracking-tight text-sm">{student.fullName}</p>
            <p className="text-[10px] text-muted-foreground font-medium">Class {student.classId}{student.sectionId}</p>
          </div>
        </div>
      )
    },
    ...
    },
    {
      ...
      cell: (student: Student) => (
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-black">{student.attendanceRate || 0}%</span>
        </div>
      )
    },
    {
