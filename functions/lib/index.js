...
exports.scheduledFeeReminders = exports.onMessageCreate = void 0;
 ...
 */
exports.scheduledFeeReminders = functions.pubsub
    ...
    try {
        ...
        if (extraDocs.length > 0) {
            console.info(`[FeeReminder] Found ${extraDocs.length} extra installments via fallback query`);
...
async function _processAllInstallments(db, docs, now, todayISO) {
...
async function _processInstallmentChunk(db, docs, now, todayISO) {
    for (const instDoc of docs) {
        ...
        if (dedupeSnap.exists)
            continue; // Already sent today's type
        try {
            // ── Resolve student → parent ───────────────────────────────────────────
            ...
            const parentUser = parentUserSnap.data();
            ...
            const reminderText = _buildReminderText(reminderType, studentName, inst.installmentName, amountFormatted, dueDateFormatted);
            // ── Find/create system→parent conversation ─────────────────────────────
            const conversationId = await _getOrCreateSystemConversation(db, parentUserId);
            ...
            await batch.commit();
            console.info(`[FeeReminder] Sent "${reminderType}" to parent ${parentUserId} for ${studentName}`);
            // ── FCM Push (non-blocking) ────────────────────────────────────────────
...
function _getReminderType(dueDateISO, now) {
...
async function _getOrCreateSystemConversation(db, parentUserId) {
    ...
    await batch.commit();
    console.info(`[FeeReminder] Created system conversation ${conversationId} for parent ${parentUserId}`);
    return conversationId;
}
// ─── Build reminder message text ─────────────────────────────────────────────
function _buildReminderText(reminderType, studentName, installmentName, amount, dueDate) {
...
async function _sendFcmToParent(parentUser, studentName, body, conversationId) {
